...
if __name__=="__main__":
    ...
    for i_im in range(100):
        ...
        # left_seed = 1015
        # right_seed = 113274
        # main_input_z = np.concatenate([np.random.RandomState(seed_main_style).randn(1, 512),
        #                                np.random.RandomState(rnd.randint(2**30)).randn(2, 512)], axis=0)
        main_input_z = np.concatenate([np.random.RandomState(seed_main_style).randn(512)[np.newaxis],
                                       np.random.RandomState(left_seed).randn(512)[np.newaxis],
                                       np.random.RandomState(right_seed).randn(512)[np.newaxis]],
                                      axis=0)
        ...
        main_model = load_network(SNAPSHOT_MAIN_PATH, (4, 8), n_styles=None, middle_input_synth=None, trunc_psi=1.6)
        print('Predicting main...')
        ...
        plt.show()
        if True:
            main_image_.save(f'imgs/s_{seed_main_style}-l_{left_seed}-r_{right_seed}-shape{main_const.shape[1]}x{main_const.shape[2]}.png')
        print('1')
    print("Success!")
