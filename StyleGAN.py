...
def StyleGAN(const_shape, n_dense, resolution, n_styles, middle_input_synth=None, TRUNCATION_PSI=0.7):
    """
    ...
    def G_synthesis(W, const_layer):
        ...
        y = None
        mod1 = ModConv2d(rank=2, name="4x4/Conv", kernel_size=3, sampling=None, filters=nf(1), padding='same')
        const_layer1 = mod1([const_layer[:, :, :const_layer.shape[2] // 2, :], W[1:2]])
        const_layer2 = mod1([const_layer[:, :, const_layer.shape[2] // 2:, :], W[2:3]])
        x = tf.concat([const_layer1, const_layer2], axis=2)
