...
def _upfirdn_2d_ref(x, k, upx, upy, downx, downy, padx0, padx1, pady0, pady1):
    ...
    # x = x[:, max(-pady0, 0) : tf.shape(x)[1] - max(-pady1, 0), max(-padx0, 0) : tf.shape(x)[2] - max(-padx1, 0), :]
...
def upsample_2d(x, k=None, factor=2, gain=1, padding=0, data_format='NCHW', impl='cuda'):
    ...
    """
    # ##########
    # return tf.keras.layers.UpSampling2D(size=2, interpolation='bilinear')(x)
    # ##########
...
#----------------------------------------------------------------------------
def upsample_conv_2d_grouped(x, w, k=None, factor=2, gain=1, padding=0, data_format='NCHW', impl='cuda'):
    r"""Fused `upsample_2d()` followed by `tf.nn.conv2d()`.
